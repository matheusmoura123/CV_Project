...
Mat get_data_vector(const Mat& descriptors) {
...
void predict_img(const Mat& img, Mat& img_keypoints, int& pred_ID, double& pred_strength, const Ptr<SVM>& svm) {
    Mat gray_img;
    Ptr<SIFT> sift_ptr = SIFT::create(0, 3, 0.04, 20, 1.6);
    ...
    Mat data_vector = get_data_vector(descriptors);
    Mat results;
...
void predict_categories(const vector<Mat>& images_to_predict, vector<food> categories, vector<int>& pred_IDs, vector<double>& pred_strengths) {
    ...
    for (int i = 0; i < number_classes; ++i) {
        read_detect_compute_img(categories[i].className, categories[i].imageNumbers, categories[i].classLable);
    }
    ...
    try {
        ...
        int clusterCount = dict_size, attempts = 5, iterationNumber = 1000;
    ...
    for (const auto & img : images_to_predict) {
        ...
        predict_img(img, dst, pred_ID, pred_strength, svm);
        /*
